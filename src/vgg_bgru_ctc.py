...
def model(is_training):
    ...
    x = Conv2D(64, (3,3), strides=(1,1), padding="same", kernel_regularizer=regularizer, kernel_initializer=initializer, use_bias=True, name='conv2d_1')(inputs) # 32*128*64 
    x = BatchNormalization(name="BN_1")(x)
    x = Activation("relu", name="relu_1")(x)
    x = MaxPooling2D(pool_size=(2,2), strides=2, padding='valid', name='maxpl_1')(x) # 16*64*64

    x = Conv2D(128, (3,3), strides=(1,1), padding="same", kernel_regularizer=regularizer, kernel_initializer=initializer, use_bias=True, name='conv2d_2')(x) # 16*64*128
    x = BatchNormalization(name="BN_2")(x)
    x = Activation("relu", name="relu_2")(x)
    x = MaxPooling2D(pool_size=(2,2), strides=2, padding='valid', name='maxpl_2')(x) # 8*32*128
    
    x = Conv2D(256, (3,3), strides=(1,1), padding="same", kernel_regularizer=regularizer, kernel_initializer=initializer, use_bias=True, name='conv2d_3')(x) # 8*32*256 
    x = BatchNormalization(name="BN_3")(x)
    x = Activation("relu", name="relu_3")(x)
    x = Conv2D(256, (3,3), strides=(1,1), padding="same", activation='relu', kernel_regularizer=regularizer, kernel_initializer=initializer, use_bias=True, name='conv2d_4')(x) # 8*32*256
    x = BatchNormalization(name="BN_4")(x)
    x = Activation("relu", name="relu_4")(x)    
    x = MaxPooling2D(pool_size=(2,1), strides=(2,1), name='maxpl_3')(x) # 4*32*256

    x = Conv2D(512, (3,3), strides=(1,1), padding="same", kernel_regularizer=regularizer, kernel_initializer=initializer, use_bias=True, name='conv2d_5')(x) # 4*32*512
    x = BatchNormalization(axis=-1, name='BN_5')(x)
    x = Activation("relu", name='relu_5')(x)
    x = Conv2D(512, (3,3), strides=(1,1), padding="same", kernel_regularizer=regularizer, kernel_initializer=initializer, use_bias=True, name='conv2d_6')(x) # 4*32*512
    ...
    x = Conv2D(512, (2,2), strides=(1,1), padding='same', kernel_regularizer=regularizer, kernel_initializer=initializer, use_bias=True, name='conv2d_7')(x) # 2*32*512
    ...
    # RNN part
    y = Bidirectional(GRU(256, return_sequences=True, kernel_initializer=initializer, kernel_regularizer=regularizer), name='GRU_1')(rnn_input) # 32*512
    y = BatchNormalization(name="BN_8")(y)
    y = Bidirectional(GRU(256, return_sequences=True, kernel_initializer=initializer, kernel_regularizer=regularizer), name='GRU_2')(y) # 32*512
                                                                       # 尝试跳过rnn层
    y_pred = Dense(num_classes, activation='softmax', name='y_pred', kernel_initializer=initializer, kernel_regularizer=regularizer)(y) # 32*11 这用来做evaluation 和 之后的test检测
