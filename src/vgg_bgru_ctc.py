...
def model(is_training):
    ...
    x = Conv2D(64, (3,3), strides=(1,1), padding="same", activation='relu', kernel_regularizer=regularizer, kernel_initializer=initializer, use_bias=True, name='conv2d_1')(inputs) # 32*128*64 
    x = MaxPooling2D(pool_size=(2,2), strides=2, padding='valid', name='maxpl_1')(x) # 16*64*64

    x = Conv2D(128, (3,3), strides=(1,1), padding="same", activation='relu', kernel_regularizer=regularizer, kernel_initializer=initializer, use_bias=True, name='conv2d_2')(x) # 16*64*128
    x = MaxPooling2D(pool_size=(2,2), strides=2, padding='valid', name='maxpl_2')(x) # 8*32*128
    
    x = Conv2D(256, (3,3), strides=(1,1), padding="same", activation='relu', kernel_regularizer=regularizer, kernel_initializer=initializer, use_bias=True, name='conv2d_3')(x) # 8*32*256 
    ...
    x = Conv2D(512, (2,2), strides=(1,1), padding='same', kernel_regularizer=regularizer, activation='relu', kernel_initializer=initializer, use_bias=True, name='conv2d_7')(x) # 2*32*512
